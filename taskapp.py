...
# Disabled for public repo as it requires a Google Service Account.
# Export progress function will not work. 
# from export_progress import copy_master_sheet, update_spreadsheet

# Disabled for public repo as it requires a SendGrid API key.
# You can create a few account and update send_grid_email.py to use one of them.
#import send_grid_email

app = Flask(__name__)

# Set secret key for Flask App. 
app.config['SECRET_KEY'] = "iMSVkHucYB5O8T4Oy0BLbGd2XT9CurNn"


# Keys for Google reCAPTCHA.
app.config['RECAPTCHA_SITE_KEY'] = '6LcbJ30dAAAAALjb1PXoPSlOxMjVkN53Gi8slavf'
app.config['RECAPTCHA_SECRET_KEY'] = '6LcbJ30dAAAAAJqzKVyaW6IqIUk38jUY-bWhSjTw' 
...
def complete_button():
    ...
    if current_task != None:
        complete_task(session['username'])
        return redirect(url_for('dashboard'))
    return redirect(url_for('dashboard'))
