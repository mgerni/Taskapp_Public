...
# Was used before making site public. NOT USED ANYMORE. 
def add_beta_keys(key_list):
    coll = db['beta_keys']
    beta_keys = []
    ...
    coll.insert_many(beta_keys)


# Was used before making site public. NOT USED ANYMORE. 
def test_keys():
    account_name = "Gerni"
    test_key = "5OertHz926kHzRGgBJOYrH6AYJ3nSzWD"
    coll = db['beta_keys']
    attempted_keys = coll.find({'status': "AVAILABLE"})
    for key in attempted_keys:
        if bcrypt.checkpw(test_key.encode('utf-8'), key['key']):
            
            coll.update_one({'_id': key['_id']},{'$set': {"status" : "USED", "owner": account_name}})
            break
